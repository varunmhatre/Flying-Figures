...
//destructor: clean up all the DirectX objects
Mesh::~Mesh(void)
{
	vbuffer->Release(); vbuffer = 0;
	ibuffer->Release(); ibuffer = 0;
}
...
Mesh::Mesh(const char* objFile, ID3D11Device * d, std::string s)
{
	...
	// create the vertex and index buffer using the vector of 
	// Vertex and Index structs we just created using helper methods
	CreateBuffers(&verts[0], vertCounter, &indices[0], vertCounter, d);
...
// returns to the pointer to the vertex buffer objects
ID3D11Buffer * const Mesh::GetVertexBuffer()
{
	return vbuffer;
}

// returns to the pointer to the index buffer objects
...
// helper methods: for DirectX buffer creation code 
void Mesh::CreateBuffers(Vertex * v, int num_v, unsigned int * i, int num_i, ID3D11Device * d)
{
	// parameters: pointer to vertex struct, number of the vertices in the struct, pointer to index
	// struct, number of the indices in the struct, device d

	// create vertex buffers

	D3D11_BUFFER_DESC vbd = {};
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(Vertex) * num_v;
	...
	d->CreateBuffer(&vbd, &initialVertexData, &vbuffer);

	// create index buffers

	D3D11_BUFFER_DESC ibd = {};
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(int) * num_i;
	...
	d->CreateBuffer(&ibd, &initialIndexData, &ibuffer);

	// Save the indices
	this->num_index = num_i;
}
