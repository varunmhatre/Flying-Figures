...
{ 
	...
	float3 normal		: NORMAL;       // normal coordination
...
VertexToPixel main( VertexShaderInput input )
{
	...
	//try to put the outline code here (amplify the position of the model)
	float Outline = 0.004;
	float2 offset = mul(float4(input.normal, 1.0f), worldViewProj).xy;
	float z = output.position.z;
	output.position.xy += offset*z*Outline;
