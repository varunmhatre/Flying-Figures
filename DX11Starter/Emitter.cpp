...
void Emitter::Update(float dt)
{
	...
	{
		timeSinceBeginning += dt;								// Time passed since beginning of the game
	}
	//printf("\n%f = Emitter %d", timeSinceBeginning, Emitter_Type);

	if(Emitter_Type == 1 && timeSinceBeginning> emitterAge)									// Stop Emitter particle system after 1 seconds
	{
		return;
	}
...
void Emitter::UpdateSingleParticle(float dt, int index)
{
	...
	// Lerp size
	// Code to Increase size of the particle
	...
	// Code line to give motion to particles
...
void Emitter::ResetEmitter(DirectX::XMFLOAT3 newPosition, float EmitterAge)
{
	timeSinceBeginning = 0;

	this->emitterPosition = newPosition;
	this->emitterAge = EmitterAge;
}
...
void Emitter::SpawnParticle()
{
	...
	//	if (Emitter_Type != 1)						// Don't execute code in if condition for Explosion because it makes looping true, but make sure "max particles value passed in emitter constructor is greater than 1 from game.cpp
//		{
			// Increment and wrap
			firstDeadIndex++;
